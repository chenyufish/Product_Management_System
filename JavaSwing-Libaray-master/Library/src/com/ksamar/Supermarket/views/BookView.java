...
import com.ksamar.Supermarket.tools.pagination.Pagination;
import com.ksamar.Supermarket.tools.placeholder.Placeholder;
import com.ksamar.Supermarket.tools.table.LibraryTableModel;
import com.ksamar.Supermarket.views.book.AddBookView;
import com.ksamar.Supermarket.views.book.DeleteBookView;
import com.ksamar.Supermarket.views.book.EditBookView;
...
public class BookView extends JPanel {
     ...
     * 属性
     */
    private String searchPlaceholder = "请输入收银员卡号";
    private String searchType = "商品名";
     ...
     * 表格模型
     */
    private String[] name = {"Id", "类别", "商品名", "厂家", "价格(人民币)", "数量", "GTIN号码"};
    ...
    public BookView() {
        ...
        searchComboBox.addItem("商品名");
        //searchComboBox.addItem("作者");
        searchComboBox.addItem("厂家");
        ...
        // 添加图书按钮
        addBookViewButton.setText("添加商品");
        ...
        // 编辑图书按钮
        editBookViewButton.setText("编辑商品");
        ...
        // 删除图书按钮
        deleteBookViewButton.setText("删除商品");
    ...
    public void getBook() {
        ...
        List<Products> productsArrayList = ProductsController.pageBook(page, size);
        tableModel.addBookRow(productsArrayList);
        // 设置分页
        setPagination();
    }
...
//                    break;
    ...
    private void searchBook() {
        ...
        tableModel.setDataVector(tableDate, name);
        String text = searchFiled.getText();
        List<Products> productsArrayList = ProductsController.searchBookLike(searchType, text, page, size);
        tableModel.addBookRow(productsArrayList);
        setPagination();
    }
    ...
    private int getPageTotal() {
        ...
        String text = searchFiled.getText();
        if (!"".equals(text) && !searchPlaceholder.equals(text)) {
            bookSize = ProductsController.searchBookLikeCount(searchType, text);
        } else {
            bookSize = ProductsController.getCount();
        }
    ...
    private void showEditBookView() {
        if (tableRow >= 0) {
            ...
            Message.showMessage("请选择需要编辑的商品", "错误", 0);
        }
    }
    ...
    private void showDeleteBookView() {
        if (tableRow >= 0) {
            ...
            Message.showMessage("请选择需要删除的商品", "错误", 0);
        }
    }
}
